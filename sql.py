...
def sql_prom_pos(identifiers,recon_date,sleeve_agg):
        # Set up the database connection parameters
        server = 'P01-01-AG-004'
        database = 'custodydata'
        username = 'ARBFUND\matthewray'
        ...
        # Execute a SQL query
        query = f'''
        SELECT DISTINCT pp.partitionID, pp.ReconDate, pp.Cusip [Custody Cusip], sm.Cusip [CW Cusip], sm.ID [Security ID]  from [data].[PrometheusPosition] pp
        JOIN reportingLM.dbo.securitymaster sm on sm.ID = pp.SecurityID
        WHERE partitionid IN ({sleeve_agg})
        AND ReconDate = '{recon_date}'
        AND sm.Cusip IN ({identifiers})
        '''
